...
const BookingForm = () => {
	...
	const getRoomPriceById = async (roomId) => {
		try {
			const response = await axios.get(`http://localhost:8080/rooms/get/${roomId}`)
			setRoomPrice(response.data.body.roomPrice)
		} catch (error) {
	...
	const handleFormSubmit = async () => {
		try {
			const confirmationCode = await axios.post(`http://localhost:8080/booking/room/${roomId}/book`,booking)
			setIsSubmitted(true)
            console.log(confirmationCode)
			navigate("/booking-success",{state:{message:"Hello,"+confirmationCode.data}})
		} catch (error) {
